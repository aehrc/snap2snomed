...
describe('Snap2SnomedHttpErrorInterceptor', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        provideMockStore({
          ...
          selectors: [
            { selector: selectToken, value: tokenMsg },
            {
              ...
              value: { isAuthenticated: true, user: { token: tokenMsg } },
            },
          ],
        }),
        ...
        TranslateService,
        ErrorNotifier,
        Snap2SnomedErrorHandler,
        Snap2SnomedHttpErrorInterceptor,
        AuthService,
  ...
  it('should intercept errors', () => {
    vi.spyOn(errorNotifier.snackBar, 'open');
  ...
  it('should intercept errors but should not notify on 401 for api calls and refresh token should be called', () => {
    ...
    vi.spyOn(authService, 'refreshAuthSession');
    httpClient.get<string>(url).subscribe(
      (data) => expect(data).toBeTruthy(),
      (error: HttpErrorResponse) => {
        expect(error).toBeTruthy();
        expect(errorNotifier.snackBar.open).not.toHaveBeenCalledWith(
          'ERROR.BACKEND_ISSUES',
          ' ',
